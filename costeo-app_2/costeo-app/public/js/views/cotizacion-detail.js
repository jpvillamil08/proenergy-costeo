...
let trabajadoresCat = [], proveedoresCat = [], plantillasCat = [];
...
export async function renderCotizacionDetail(content, state, id) {
  ...
  const [f, trabs, provs, plantillas] = await Promise.all([
  ...
  full = f; trabajadoresCat = trabs; proveedoresCat = provs; plantillasCat = plantillas;
  paint(content);
}
